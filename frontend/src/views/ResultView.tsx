        ...
        <div className="pattern-exam min-h-screen py-10">
            ...
            <div className="page-shell max-w-4xl">
                <div className="text-center mb-8">
                    ...
                    <div className="mt-4 h-[3px] w-20 mx-auto" style={{ background: "var(--oq-red)" }} />
                </div>
                        ...
                        <CardContent className="gap-8">
                                    ...
                                    return (
                                        ...
                                        >
                                            <span
                                                ...
                                                style={{ fontSize: idx === 0 ? 30 : 22, color: "var(--oq-red)" }}
                                            >
                                                {p.score.toLocaleString()}
                                            </span>
                                            <span className={`font-bold truncate max-w-full ${idx === 0 ? "text-lg" : ""}`} title={p.name}>
                                                {p.name}
                                            </span>
                                            <div
                                                className="w-full mt-2 border-x-2 border-t-2 flex items-start justify-center pt-2"
                                                style={{ height: heights[idx], borderColor: "var(--oq-red)", background: "var(--oq-row-alt)", borderRadius: "12px 12px 0 0" }}
                                            >
                                                <span className="mono font-extrabold" style={{ fontSize: idx === 0 ? 44 : 30, color: "var(--oq-red)", lineHeight: 1 }}>
                                                    {medal[idx]}
                                                </span>
                                            </div>
                                        </fm.div>
                            ...
                            {rest.length > 0 && (
                                <fm.ol ref={listRef} className="flex flex-col gap-2">
                                    {rest.map((r) => (
                                        <fm.li key={r.uuid} layout className="flex items-center justify-between px-4 py-3 border border-default-200 rounded-xl bg-default-100">
                                            <span className="flex items-center gap-3">
                                                <span className="w-7 h-7 mono text-sm flex items-center justify-center border border-default-200 rounded-md">{r.rank}</span>
                                                {r.name}
                                            </span>
                        ...
                        <CardContent className="p-6">
                            ...
                            <div className="flex flex-col gap-3">
                                {review.questions.map((q, i) => (
                                    <div key={q.questionId} className="border border-default-200 rounded-xl p-4">
                                        <div className="flex items-start justify-between gap-4">
                                            <div className="flex-1">
                                                <p className="font-bold">{i + 1}. {q.title}</p>
                                                <p className="text-sm text-default-500 mt-1">{q.questionType.replace(/_/g, " ")} · {q.pointsBase} pts</p>
                                            </div>
                                            <div className="text-right">
                                                <p className="mono text-sm">
                                                    <span className={q.correctRate >= 0.7 ? "text-green-600" : q.correctRate >= 0.4 ? "text-yellow-600" : "text-red-600"}>
                                                        {Math.round(q.correctRate * 100)}%
                                                    </span>
                                                    {" "}correct
                                                </p>
                                                <p className="text-xs text-default-500">{q.totalAttempts} attempts</p>
                                            </div>
                                        </div>
                                        {q.answer != null && (
                    ...
                    <Card className="bg-content1">
                        <CardContent className="p-6">
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <Button size="sm" variant="outline" onPress={() => setNamesRevealed(!namesRevealed)}>
                                ...
                                {review.players
                                    ...
                                    .map((p, i) => (
                                        <div
                                            ...
                                            onClick={() => setSelectedStudent(selectedStudent === p.playerUuid ? null : p.playerUuid)}
                                        >
                                            <div className="flex items-center justify-between">
                                                <span className="font-bold">
                                                    {i + 1}. {namesRevealed ? p.playerName : `Player ${i + 1}`}
                                                </span>
                                                <span className="mono font-semibold">{p.totalScore} pts</span>
                                            </div>
                                                    ...
                                                    {p.answers.map((a) => (
                                                        <div key={a.questionId} className="flex items-center justify-between text-sm">
                                                            <span className={a.correct ? "text-green-600" : "text-red-600"}>
                                                                {a.correct ? "✓" : "✗"} {a.questionId}
                                                            </span>
                                                            <span className="mono text-default-500">+{a.scoreEarned}</span>
                                                        </div>
                                                    ))}
                                                </div>
                                            )}
                        ...
                        <CardContent className="p-6">
                            ...
                            <div className="grid sm:grid-cols-3 gap-4 mb-6">
                                ...
                                <div className="border border-default-200 rounded-xl p-4 text-center">
                                    ...
                                    <p className="stat-value">{Math.round(review.classStats.avgScore)}</p>
                                </div>
                                <div className="border border-default-200 rounded-xl p-4 text-center">
                                    <p className="label-caps mb-1">Correct Rate</p>
                                    <p className="stat-value">
                                        {review.classStats.totalAttempts > 0
                                            ? Math.round((review.classStats.totalCorrect / review.classStats.totalAttempts) * 100)
                                            : 0}%
                                    </p>
                                </div>
                            </div>
                                ...
                                {review.questions
                                    ...
                                    .map((q) => (
                                        <div key={q.questionId} className="flex items-center gap-3 text-sm">
                                            <div className="w-32 truncate font-medium">{q.title}</div>
                                            <div className="flex-1 h-4 bg-default-200 rounded-full overflow-hidden">
                                                <div
                                                    className="h-full rounded-full transition-all"
                                                    style={{
                                                        width: `${q.correctRate * 100}%`,
                                                        background: q.correctRate >= 0.7 ? "var(--color-kahoot-green)" : q.correctRate >= 0.4 ? "var(--color-kahoot-yellow)" : "var(--oq-red)",
                                                    }}
                                                />
                                            </div>
                                            <span className="mono text-xs w-12 text-right">{Math.round(q.correctRate * 100)}%</span>
                                        </div>
                                    ))}
                            </div>
                        </CardContent>
