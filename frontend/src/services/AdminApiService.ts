...
export class AdminApiService {
  ...
  addQuestion(q: QuestionPayload) {
    return this.client.post<QuestionPayload>(`/admin/quizzes/${q.quizId}/questions`, q).then((r) => r.data);
  }
  ...
  importBank(json: string, replace: boolean) {
    return this.client.post<{ importedQuestions: number }>("/admin/import", { json, replace }).then((r) => r.data);
  }
}
